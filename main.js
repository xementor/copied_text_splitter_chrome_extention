...
const indexDB = new IndexDB('my-database', 'prev');

indexDB.open()
  .then(() => {
    // Add an item
    // indexDB.add({'prev': 100})
    ...
    //     item.prev = 'jane';
    //     indexDB.update(item)
    //       .then(() => console.log(`Updated item with ID ${item}.`));
    //   });

    indexDB.lastKey()
      .then( i => indexDB.getById(i).then((data) => console.log(data)))
